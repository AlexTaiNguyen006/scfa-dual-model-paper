    ...
    for cond, (ac, ppa, but) in COND_MAP.items():
        with model:
            ...
            for canonical_id, label, group, candidates in PATHWAY_RXNS:
                rxn  _find_rxn(model, candidates, silentTrue)
                if rxn:
        ...
        for frac in [0.90, 0.95, 1.00]:
            with model:
                ...
                fva_result  flux_variability_analysis(
                    model, reaction_listfva_targets,
                    fraction_of_optimumfrac)
...
def main():
    ...
    pd.DataFrame(ratio_rows).to_csv(
        paths.results / "ratio_sensitivity.csv", indexFalse)
    pd.DataFrame(pfba_rows).to_csv(
        paths.results / "pfba_comparison.csv", indexFalse)
    pd.DataFrame(fva_rows).to_csv(
        paths.results / "fva_multi_threshold.csv", indexFalse)
    pd.DataFrame(cap_rows).to_csv(
        paths.results / "flux_cap_sensitivity.csv", indexFalse)
