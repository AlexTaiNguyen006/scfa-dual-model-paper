...
def cross_validate(community_df, pipeline_df):
    ...
    for _, crow in community_df.iterrows():
        ...
        dists  np.linalg.norm(pipe_vals - cvec, axis1)
...
def fig_cross_validation(community_df, pipeline_df, figs_dir):
    ...
    for _, row in pipeline_df.iterrows():
        ...
        ax1.axhline(total, colorPAL["pipeline"], ls"--", lw1.2, alpha0.6)
        ax1.text(len(community_df) - 0.3, total + 0.3, f"Pipeline: {label}",
                 fontsize8, colorPAL["pipeline"], ha"right")
    ...
    for i, (_, crow) in enumerate(community_df.iterrows()):
        ...
        ax1.bar(i, total, colorcolor, alpha0.8, width0.7)

    ax1.set_xticks(range(len(community_df)))
    labels  community_df["source"].apply(lambda s: s.replace("_", "\n")).tolist()
    ax1.set_xticklabels(labels, fontsize7, rotation0)
    ...
    legend_elements  [
        Patch(facecolorPAL["agora2"], label"AGORA2"),
        Patch(facecolorPAL["micom"], label"MICOM"),
        Patch(facecolorPAL["lit"], label"Literature"),
        plt.Line2D([0], [0], colorPAL["pipeline"], ls"--", label"Pipeline doses"),
    ]
    ax1.legend(handleslegend_elements, fontsize8)
    ...
    for i, (_, crow) in enumerate(community_df.iterrows()):
        ...
        ax2.bar(i, ac_frac, color"#0072b2", width0.7)
        ax2.bar(i, pp_frac, bottomac_frac, color"#e69f00", width0.7)
        ax2.bar(i, bt_frac, bottomac_frac + pp_frac, color"#009e73", width0.7)
    ...
    for j, (_, row) in enumerate(pipeline_df.iterrows()):
        ...
        idx  n + j
        ax2.bar(idx, ac_frac, color"#0072b2", width0.7, alpha0.5, hatch"//")
        ax2.bar(idx, pp_frac, bottomac_frac, color"#e69f00", width0.7,
                alpha0.5, hatch"//")
        ax2.bar(idx, bt_frac, bottomac_frac + pp_frac, color"#009e73",
                width0.7, alpha0.5, hatch"//")
    ...
    ax2.set_xticklabels(all_labels, fontsize7)
    ...
    ax2.legend(handlesratio_legend, fontsize8)
